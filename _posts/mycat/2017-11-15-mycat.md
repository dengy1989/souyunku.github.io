...
<mycat:schema xmlns:mycat="http://io.mycat/">
    ...
    <schema name="test_schema" checkSQLschema="false" sqlMaxLimit="100">
        <table name="test_one" primaryKey="id" dataNode="dn1,dn2,dn3" rule="sharding-by-date" />
        <table name="test_two" primaryKey="id" dataNode="dn1,dn2,dn3" rule="sharding-by-date" />
        <table name="test_three" primaryKey="id" dataNode="dn1,dn2,dn3" rule="sharding-by-date" />
    </schema>
    <!-- 设置dataNode 对应的数据库,及 mycat 连接的地址dataHost -->
    <dataNode name="dn1" dataHost="dh_test" database="db_one" />
    <dataNode name="dn2" dataHost="dh_test" database="db_two" />
    <dataNode name="dn3" dataHost="dh_test" database="db_three" />
...
<mycat:rule xmlns:mycat="http://io.mycat/">
    ...
    <function name="sharding-by-date-day" class="io.mycat.route.function.PartitionByMonth">
        <property name="dateFormat">yyyy-MM-dd</property>
        <property name="sBeginDate">2017-11-16</property>
...
- `sBegin Date` ：开始日期 
- `sEnd Date`：结束日期 
- `sPartion Day` ：分区天数，即默认从开始日期算起，分隔 5 天一个分区 
...
+------+------------------+-------+-------+--------+------+------+---------+------------+----------+---------+---------------+
| dn1  | dh_test/db_one   |     0 | mysql |      0 |    3 | 1000 |      68 |          0 |        0 |       0 |            -1 |
| dn2  | dh_test/db_two   |     0 | mysql |      0 |    3 | 1000 |      30 |          0 |        0 |       0 |            -1 |
| dn3  | dh_test/db_three |     0 | mysql |      0 |    4 | 1000 |     159 |          0 |        0 |       0 |            -1 |
+------+------------------+-------+-------+--------+------+------+---------+------------+----------+---------+---------------+
3 rows in set
...
+--------+-------+-----------------+------+---------+-------+--------+---------+--------------+---------------------+-------+
| hostM1 | mysql | 192.168.252.121 | 3306 |       1 |     0 | idle   |       0 | 1,1,1        | 2017-11-16 11:15:40 | false |
| hostS2 | mysql | 192.168.252.122 | 3306 |       1 |     0 | idle   |       0 | 1,1,1        | 2017-11-16 11:15:40 | false |
+--------+-------+-----------------+------+---------+-------+--------+---------+--------------+---------------------+-------+
2 rows in set
